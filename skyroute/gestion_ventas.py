...
def ver_ventas(conexion):
    ...
    query = '''
        SELECT v.id, v.fecha, v.estado, v.monto, v.fecha_anulacion,
...
def agregar_venta(conexion):
    ...
    cursor = conexion.cursor()
    # Mostrar clientes
    cursor.execute("SELECT id, razon_social, cuit FROM clientes")
...
def boton_arrepentimiento(conexion):
    ...
    cursor.execute('''
        SELECT v.id, v.fecha, v.estado, v.monto, c.razon_social, ciu.nombre, p.nombre
        FROM ventas v
        JOIN clientes c ON v.cliente_id = c.id
        JOIN destinos d ON v.destino_id = d.id_destino
        JOIN ciudades ciu ON d.id_ciudad = ciu.id_ciudad
    ...
    if not ventas_recientes:
        print("No hay ventas recientes para anular (solo se permite anular dentro de los últimos 5 minutos).")
        cursor.close()
        return
    ...
    while True:
        try:
            ...
            if confirmacion.upper() == 'SI':
                ...
                cursor.execute("UPDATE ventas SET estado = 'Anulada', fecha_anulacion = %s WHERE id = %s", (fecha_anulacion, venta_id))
                conexion.commit()
